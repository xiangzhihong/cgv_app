...
const AdvertisingBanner = ({ thatCd, len, isLoggedIn, type, navigation }) => {
    ...
    return (
      <Continer onPress={onPress}>
        <Image
          ...
          style={[{ height }, style]}
        />
        {type === 'APP_SY_FOOT_AD' ?<Text style={{textAlign: 'center', paddingTop: 10, paddingBottom: 8}}>{item.advertTitle}</Text>:null}
      </Continer>
    )
  }
  ...
  const onPressBanner = (linkObjectInfo = '') => {
    ...
    if (first === IntegralMall) {
      ...
      navigate('PersonalTopicScreen', { id: second, title: `${second} 话题讨论` })
  ...
  return (
    !option.isFlatList ? (
      <View style={{ height: option.height }}>
        ...
        >
          {list.map((item, index) => len && index > 1 ? null : <Banner key={item.id} item={item} height={option.height} style={option.bannerStyle} onPress={() => onPressBanner(item.advertImgLinkurl)} />)}
        </Swiper>
      </View>
    ) :
      <FlatList
        data={list}
        renderItem={({ item }) => <Banner key={item.id} item={item} height={option.height} style={option.bannerStyle} onPress={() => onPressBanner(item.advertImgLinkurl)} />}

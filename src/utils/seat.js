...
export const canSelect = (selfCoord = {}, seatRow = [], selected=[], isCancel = false) => {
  const { row, column } = selfCoord
  const copySeatRow = seatRow.map((item, index) => index)
  const selectedSameRow = selected.filter(val => val.row === row).map(item => item.column).sort((a, b) => a-b)
  ...
  const res = getTrgetSelect(selectedSameRow, splitAvailableSeat, targetAvailableAreaIndex)  // 在即将选中的区块中的已选中位置
  ...
  if(isCancel){
    ...
    for(let i=0;i<splitSelectData.length;i++){
      if(column === splitSelectData[i][0] || column === splitSelectData[i][splitSelectData[i].length - 1]){
        // 从已选的座位两端可以取消
        console.log('可以')
        return true
      }
  ...
  if(targetSelectAreaLength === 3){
    if(res.length > 0){
      ...
      const isOrderlyData = isOrderly(targetSelectArea, res, res[0] !== targetSelectArea[0])
      if(isOrderlyData) {
        console.log('可以')
        return true
      }
  ...
  if(res.length === 0) {
    ...
    if(column - targetSelectArea[0] > 1 && targetSelectArea[targetSelectArea.length - 1] - column > 1 ){
      // 大于左边两格 小于右边两格
      console.log('可以')
      return true
    }
...
const getAvailable = (seatRow, targetRow) => {
  ...
  for(let i = 0; i< targetRow.length; i++) {
    ...
    if(seatRow[current]){
      if(seatRow[current].status === 'Available' || seatRow[current].status === 'Yellow') {
        arr.push(current)
      }
    }
  }
...
const getTrgetSelect = (selectedSameRow, splitAvailableSeat, targetAvailableAreaIndex) => {
  const targetSelect = []
